...
class WaterMask{
    ...
    public  $xposition          = '';                  //水印文字自定义x位置
    public  $yposition          = '';                  //水印文字自定义y位置
    ...
    private function waterstr() {
        ...
        imagettftext($this->water_im, $this->fontSize, 0, $this->x, $this->y, $color, $this->fontFile, $this->waterStr);
        $this->waterImg_info = array(0=>$w,1=>$h);
        $this->waterimg($type='waterstr');
    }

    private function waterstrPerson() {
        $rect = imagettfbbox($this->fontSize,0,$this->fontFile,$this->waterStr);
        $w = abs($rect[2]-$rect[6]);
        $h = abs($rect[3]-$rect[7]);
        $fontHeight = $this->fontSize;
