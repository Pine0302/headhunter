...
!function ($) {
    ...
    var initCommonSearch = function (pColumns, that) {
        ...
        // 导出功能
        form.on("click", "button[type=export]", function (event) {
            event.preventDefault();
            that.onCommonExport();
            return false;
        });
    ...
    var createFormBtn = function (that) {
        ...
        var searchExport = that.options.formatCommonExportButton();
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        onCommonExport: function (field, text) {
            return false;
        },
    ...
    $.extend($.fn.bootstrapTable.Constructor.EVENTS, {
        ...
        'common-export.bs.table': 'onCommonExport',
    ...
    BootstrapTable.prototype.onCommonExport = function () {
        //console.log(this.options.extend.index_url);
        var searchQuery = getSearchQuery(this);

        if(this.options.extend.index_url=="re/usercomp/index"){
            var index_url_arr = this.options.extend.index_url.split("/");
            var re_company_id = searchQuery.filter.re_company_id;
            var status = searchQuery.filter.status;
            var redirect_url = index_url_arr[0]+'/'+index_url_arr[1]+'/exportNew?re_company_id='+re_company_id+"&status="+status;
            var redirect_url = index_url_arr[1]+'/exportNew?re_company_id='+re_company_id+"&status="+status;
            window.location.href = redirect_url;
        }else{
            alert('该版本中此功能仅限公司人员管理中使用!');
        }
        this.trigger('common-export', this, searchQuery);
        this.options.pageNumber = 1;
      //  this.refresh({});
    };
