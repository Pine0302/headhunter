define(['jquery', 'bootstrap', 'moment', 'bootstrap-table-commonsearch','moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export',  'bootstrap-table-template'], function ($, undefined, Moment,SearchC) {
    var Table = {
        list: {},
        ...
        api: {
            init: function (defaults, columnDefaults, locales) {
                ...
                $.extend($.fn.bootstrapTable.locales[Table.defaults.locale], {
                    ...
                    formatCommonExportButton: function () {
                        return "导出";
                    },
            ...
            bindevent: function (table) {
                ...
                $(toolbar).on('click', Table.config.exportbtn, function () {
                    ...
                    var that = this;
                  //  console.log(table.bootstrapTable.defaults('queryParams'));
                    console.log(SearchC.getSearchQuery);
                ...
                //导出按钮事件
           /*     if ($(Table.config.exportbtn, toolbar).size() > 0) {

                    table.on('export.bs.table', function (e, settings, data) {
                        $(Table.config.exportbtn, toolbar).find(".fa").addClass("fa-spin");
                    });
                    Fast.api.ajax({
                ...
                $(toolbar).on('click', Table.config.passbtn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    Layer.confirm(
                        __('Are you sure you want to pass the %s selected item?', ids.length),
                        {icon: 3, title: __('Warning'), offset: 0, shadeClose: true},
                        function (index) {
                            Table.api.multi("pass", ids, table, that);
                            Layer.close(index);
                        }
                    );
                });
            ...
            multi: function (action, ids, table, element) {
                ...
                var url = typeof data.url !== "undefined" ? data.url : (action == "del" ? options.extend.del_url : (action == 'pass' ? options.extend.pass_url :(action == 'deny' ? options.extend.deny_url :  (action=='detail'  ? options.extend.detail_url : (action == 'online' ? options.extend.online_url :  (action=='offline'  ? options.extend.offline_url : options.extend.multi_url )) ))));
            ...
            events: {
                operate: {
                    ...
                    'click .btn-passone': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
                        ...
                        Layer.confirm(
                            __('Are you sure you want to pass this item?'),
                            {icon: 3, title: __('Warning'), offset: [top, left], shadeClose: true},
                            function (index) {
                                var table = $(that).closest('table');
                                var options = table.bootstrapTable('getOptions');
                                Table.api.multi("pass", row[options.pk], table, that);
                                Layer.close(index);
                            }
                        );
                    },
                    'click .btn-promptone': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
               ...
               /*         Layer.confirm(
                        ...
                        layer.prompt({
                            ...
                            var table = $(that).closest('table');
                            var options = table.bootstrapTable('getOptions');
                            Table.api.multi("prompt", row[options.pk], table, that);
                            Layer.close(index);
                        });
                    },
                    'click .btn-onlineone': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
                        ...
                        Layer.confirm(
                            ...
                            function (index) {
                                var table = $(that).closest('table');
                                var options = table.bootstrapTable('getOptions');
                                Table.api.multi("online", row[options.pk], table, that);
                                Layer.close(index);
                            }
                        );
                    },
                    'click .btn-offlineone': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
                        ...
                        Layer.confirm(
                            ...
                            function (index) {
                                var table = $(that).closest('table');
                                var options = table.bootstrapTable('getOptions');
                                Table.api.multi("offline", row[options.pk], table, that);
                                Layer.close(index);
                            }
                        );
                    },
                    'click .btn-denyone': function (e, value, row, index) {
                        ...
                        Layer.confirm(
                            __('Are you sure you want to deny this item?'),
                            {icon: 3, title: __('Warning'), offset: [top, left], shadeClose: true},
                            function (index) {
                                var table = $(that).closest('table');
                                var options = table.bootstrapTable('getOptions');
                                Table.api.multi("deny", row[options.pk], table, that);
                                Layer.close(index);
                            }
                        );
                    }
                ...
                $.each(buttons, function (i, j) {
                   /* console.log(i);
                    ...
                    if (type === 'operate') {
                        ...
                        if (['add', 'edit', 'del','pass','detail','deny', 'multi', 'dragsort'].indexOf(j.name) > -1 && !options.extend[j.name + "_url"]) {
                            return true;
                        }
                    }
