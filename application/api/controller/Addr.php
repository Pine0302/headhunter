...
//    /protected $noNeedRight = ['test2'];
    ...
    public function chooseDistrict(){
        ...
        $sess_key = $request_data['sess_key'];
        $lat = isset($request_data['lat']) ? $request_data['lat'] :$this->config_area['user_default_position']['lat'];
        $lng = isset($request_data['lng']) ? $request_data['lng'] :$this->config_area['user_default_position']['lng'];
        $prov_arr = Db::table('areas')
            ->where(['arealevel'=>1])
            ->where(['typename'=>'省'])
            ->select();
        $prov_list = [];
        foreach($prov_arr as $kp=>$vp){
            $areaname = (mb_strlen($vp['areaname']) > 4) ? mb_substr($vp['areaname'],0,2) : $vp['areaname'];
            $prov_list[] = [
                'prov_code'=>$vp['areano'],
                'prov_name'=>$areaname,
            ];
        ...
        $self_prov = [
            'prov_code'=>$this->config_area['direct_city_code'],
            'prov_name'=>'直辖市',
        ];
        array_unshift($prov_list,$self_prov);

        //获取用户的城市访问足迹
        $rencent_visit = [];
        $footprint_set = config('webset.footprint');
        $user_info  = $this->getGUserInfo($sess_key);

        $footprint_arr = Db::table('re_footprint')
            ->where('user_id','=',$user_info['id'])
            ->where('type','=',$footprint_set['type'])
            ->order('update_at desc')
            ->select();
        if(!empty($footprint_arr)){
            foreach($footprint_arr as $kf=>$vf){
                $city_info = unserialize($vf['content']);
                $rencent_visit[] = $city_info;
            }
        }
        ...
        $user_position_info = $CommonFuncObj->getUserPostition($lat,$lng,$sess_key);
        if(!empty($user_position_info['city_code'])&&(!empty($user_position_info['city_name']))){
            $position_city = [
                'city_code'=>$user_position_info['city_code'],
                'city_name'=>$user_position_info['city_name'],
            ];
        }
        //获取热门城市
        $hot_city = [];
        $hot_city = $this->config_area['hot_city_list'];
    ...
    //省份到城市
    public function prov2CityList(){
        $data = $this->request->post();
        $sess_key = $data['sess_key'] ?? '';
        $prov_code = $data['prov_code'];
        $city_list = [] ;
        if($prov_code==$this->config_area['direct_city_code']){     //直辖市
            $city_list = $this->config_area['direct_city_list'];
        }else{
            $city_arr = Db::table('areas')
                ->where('parentno','=',$prov_code)
                ->where('arealevel','=',2)
                ->select();
            foreach($city_arr as $kc=>$vc){
                $city_list[] = [
                    'city_code' => $vc['areano'],
                    'city_name' => $vc['areaname'],
                ];
            }
        }
        $response_data = [
    ...
    //城市到区域
    public function city2DistrictList(){
        $data = $this->request->post();
        $sess_key = $data['sess_key'] ?? '';
        ...
        $district_arr = Db::table('areas')
            ...
            ->where('arealevel','=',3)
            ->select();
        foreach($district_arr as $kc=>$vc){
            $district_list[] = [
                'district_code' => $vc['areano'],
                'district_name' => $vc['areaname'],
            ];
        }
        $response_data = [
            'data'=>[
                'area_list'=>$district_list,
            ],
        ];
        $this->success('success', $response_data);
    }
