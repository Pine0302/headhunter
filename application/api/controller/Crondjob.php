...
class Crondjob extends Api
{
    ...
    //定时更新投票结果
    public function topicEnd(){
        //查询当前到期的topic
        $topic_id = 2;
        $time = time();
        $time_date = date("Y-m-d H:i:s");
        $one_year_later = $time+ 365*24*60*60;
        $topicQuery = Db::table('re_topic');
        $end_topic_list = $topicQuery
            ->where('result','=',0)
            ->where('end_time','<',$time)
            ->select();
        $topicQuery->removeOption();
        foreach($end_topic_list as $ke=>$ve){
            $topic_id = $ve['id'];


            //将这个话题状态关闭
            $topicQuery = Db::table('re_topic');
            $topicQuery->where('id','=',$topic_id)->update(['result'=>'-1']);
            $topicQuery->removeOption();

            //查看这个话题的详情
            $topicQuery = Db::table('re_topic');
            $topic_info = $topicQuery->where('id','=',$topic_id)->find();
            $topicQuery->removeOption();
            $total_num = $topic_info['total_num'];
            $total_coin = $topic_info['total_coin'];
            $blue_num = $topic_info['blue_num'];
            $red_num = $topic_info['red_num'];
            //var_dump($topic_info);exit;
            if($total_num>0){
                $userServiceObj = new UserService();
                //获取所有参与人员
                $vote_list =  Db::table('re_topic_vote')->where('re_topic_id','=',$topic_id)->select();
                /*$blue_user_ids = [];
                $red_user_ids = [];
                foreach($vote_list as $kv=>$vv){
                    if($vv['vote']==1){
                        $blue_user_ids[] = $vv['user_id'];
                    }else{
                        $red_user_ids[] = $vv['user_id'];
                    }
                }*/
                //判断胜负 1.蓝方胜 2.红方胜 3.平局
                if($blue_num>$red_num){   //蓝方胜利
                    //修改topic
                    $change_topic_result = Db::table('re_topic')->where('id','=',$topic_id)->update(['result'=>1]);
                    Db::table('re_topic')->removeOption();
                    //修改每个参与者的re_topic_vote
                    $change_topic_vote_blue = Db::table('re_topic_vote')
                        ->where('re_topic_id','=',$topic_id)
                        ->where('vote','=',1)
                        ->update(['result'=>1]);
                    Db::table('re_topic_vote')->removeOption();
                    $change_topic_vote_red = Db::table('re_topic_vote')
                        ->where('re_topic_id','=',$topic_id)
                        ->where('vote','=',2)
                        ->update(['result'=>2]);
                    Db::table('re_topic_vote')->removeOption();
                    //给蓝方user 增加coin    +  //给蓝方user增加coin 记录
                    $coin_gain = floor($total_coin/$blue_num);
                    foreach($vote_list as $kv=>$vv){
                        if($vv['vote']==1){  //投了蓝方的用户
                            $userServiceObj->changeUserCoin($vv['user_id'],$vv['user_type'],$coin_gain,NUM_INC);
                            $coin_log = [
                                'user_id'=>$vv['user_id'],
                                ...
                                'way'=>1,
                                'method'=>3,
                                're_topic_id'=>$topic_id,
                                'status'=>1,
                                'left_coin'=>$coin_gain,
                                'create_at'=>$time_date,
                                'update_at'=>$time_date,
                                'expire_at'=>$one_year_later,
                                'admin_id'=>1,
                            ];
                            Db::table('re_coin_log')->insert($coin_log);
                        }
                    }
                }elseif ($blue_num<$red_num){    //红方胜利

                    //修改topic
                    $change_topic_result = Db::table('re_topic')->where('id','=',$topic_id)->update(['result'=>2]);
                    Db::table('re_topic')->removeOption();
                    //修改每个参与者的re_topic_vote
                    $change_topic_vote_blue = Db::table('re_topic_vote')
                        ->where('re_topic_id','=',$topic_id)
                    ...
                    $change_topic_vote_red = Db::table('re_topic_vote')
                        ...
                        ->update(['result'=>1]);
                    Db::table('re_topic_vote')->removeOption();
                    //给蓝方user 增加coin    +  //给红方user增加coin 记录
                    $coin_gain = floor($total_coin/$red_num);
                    foreach($vote_list as $kv=>$vv){
                        if($vv['vote']==2){  //投了红方的用户
                            $userServiceObj->changeUserCoin($vv['user_id'],$vv['user_type'],$coin_gain,NUM_INC);
                            $coin_log = [
                                'user_id'=>$vv['user_id'],
                                'user_type'=>$vv['user_type'],
                    ...
                    $topicQuery = Db::table('re_topic');
                    $topic_info = $topicQuery->where('id','=',$topic_id)->update(['result'=>'3']);  //平局
                    $topicQuery->removeOption();
                    Db::table('re_topic_vote')
                        ->where('re_topic_id','=',$topic_id)
                        ->update(['result'=>3]);
                    //给所有参与者 增加coin    +  //增加coin 记录
                    $coin_gain = $topic_info['coin'];
                    foreach($vote_list as $kv=>$vv){
                        $userServiceObj->changeUserCoin($vv['user_id'],$vv['user_type'],$coin_gain,NUM_INC);
                        $coin_log = [
                            'user_id'=>$vv['user_id'],
