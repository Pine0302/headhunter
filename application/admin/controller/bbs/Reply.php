...
class Reply extends Backend
{
    ...
    /**
     * BbsReply模型对象
     * @var \app\admin\model\BbsReply
     */
    protected $model = null;
    protected $relationSearch = true;
    protected $searchFields = 'bbsMember.nick_name,bbsMemberFor.nick_name,content';

    public function _initialize()
    {
        parent::_initialize();
        $this->model = model('BbsReply');
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                ->with('bbsMember,bbsMemberFor,bbsPost')
                ->where($where)
                ->order($sort, $order)
                ->count();
            $this->model->removeOption();
            ...
            foreach($list as $kl=>$vl){
              ...
              //  $list[$kl]['bbsForum'] = $vl['bbs_forum'];
                ...
                $list[$kl]['bbsMemberFor'] = $vl['bbs_member_for'];
                $list[$kl]['bbsPost'] = $vl['bbs_post'];
                $list[$kl]['ch_status'] = ($vl['status']==1) ? '隐藏':'展示';
